...
export const seed = async (knex: Knex): Promise<any> => {
  ...
  for (let i = 0; i < TOTAL_ENTERIES; i++) {
    const [user_id] = await knex(TABLE.USER).insert(
      {
    ...
    for (let j = 0; j < TOTAL_ENTERIES; j++) {
      const [post_id] = await knex(TABLE.POST).insert(
        { user_id: user_id, description: Faker.lorem.text() },
        ['id'],
      );
      for (let k = 0; k < TOTAL_ENTERIES; k++) {
        await knex(TABLE.COMMENT).insert(
          { post_id: post_id, message: Faker.lorem.text() },
          ['id'],
        );
      }
    }
