...
export class AppService {
  ...
  getInfo(configService: ConfigurationService): Record<string,any> {
    ...
    links.links['health'] =`${configService.APPLICATION_HOST}/${configService.APP.API_URL_PREFIX}/health`;
    if(configService.SWAGGER.ENABLE) {
      links.links['swagger'] =`${configService.APPLICATION_HOST}${configService.SWAGGER.ROUTE}`;
    }
    if(configService.GRAPHQL.PLAYGROUND) {
      links.links['graphql'] =`${configService.APPLICATION_HOST}${configService.GRAPHQL.ROUTE}`;
    }
    return {
