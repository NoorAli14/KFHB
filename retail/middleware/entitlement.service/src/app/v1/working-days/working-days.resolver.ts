...
import {
  WorkingDayCreateInput,
  WorkingDayInput,
} from '@app/v1/working-days/working-day.dto';
...
export class WorkingDaysResolver {
  ...
  @Query(() => WorkingDay)
  async findWorkingDayById(
    @Args('id') id: string,
    @Fields() output: string[],
    @CurrentUser() current_user: ICurrentUser,
  ): Promise<WorkingDay> {
    const workingDay: WorkingDay = await this.workingDaysService.findById(
      current_user,
      id,
      output,
    );
  ...
  @Mutation(() => WorkingDay)
  async addWorkingDay(
    @Args('input') input: WorkingDayCreateInput,
    @Fields() output: string[],
    @CurrentUser() current_user: ICurrentUser,
  ): Promise<WorkingDay> {
