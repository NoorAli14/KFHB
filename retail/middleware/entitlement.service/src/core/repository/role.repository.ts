...
export class RoleRepository extends BaseRepository {
  ...
  getQuery(
    ...
    if (filteringParams?.name)
      query = query.where('name', 'like', `%${filteringParams.name}%`);
  ...
  async listRolesByUserID(userIds: readonly string[]): Promise<any> {
    const condition = {};
    ...
    try {
      ...
      if (module_permission_ids?.length > 0) {
        ...
        for (const mpi of module_permission_ids) {
          mpi._deleted
            ? modulePermissionsToDelete.push(mpi.id)
            : newModulePermissions.push(mpi.id);
        }
        ...
        const parentModulePermissions = await this.getParentModules(
        ...
        const idsAlready = await trx(TABLE.MODULE_PERMISSION_ROLE)
        ...
        idsAlready.forEach(idObj => {
          newModulePermissions.splice(
            newModulePermissions.indexOf(idObj.module_permission_id || idObj),
            1,
          );
        });
  ...
  async create(role: Record<string, any>, keys: string[]): Promise<Role[]> {
    ...
    try {
      ...
      if (module_permissions) {
        const module_permission_roles = module_permissions.map(
          module_permission => {
            return {
              role_id: response[0].id || response[0],
        ...
        const parentModulePermissions = await this.getParentModules(
          module_permissions.map(mp => mp.id),
        );
        parentModulePermissions.map(parentModulePermission => {
