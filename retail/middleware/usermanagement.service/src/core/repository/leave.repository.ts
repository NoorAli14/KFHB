...
export class LeaveRepository extends BaseRepository {
  ...
  getQuery(
    ...
    if (filteringParams?.leave_type_id)
      query = query.where(
        'leave_type_id',
        'like',
        `%${filteringParams.leave_type_id}%`,
      );
    if (filteringParams?.status)
      query = query.where('status', '=', filteringParams.status);
    if (filteringParams?.start_date)
      query = query.where('start_date', '=', filteringParams.start_date);
    if (filteringParams?.end_date)
      query = query.where('end_date', '=', filteringParams.end_date);
