...
export class PermissionLoader {
  ...
  async findPermissionsByModuleID(keys: readonly string[]): Promise<any> {
    const permissions: any = await this.permissionDB.listPermissionsByModuleID(
      keys,
    );
...
@Injectable()
export class PermissionsDataLoader
  implements NestDataLoader<string, Permission> {
  constructor(private readonly loader: PermissionLoader) {}

  generateDataLoader(): DataLoader<string, Permission> {
    return new DataLoader<string, Permission>(keys =>
      this.loader.findPermissionsByModuleID(keys),
    );
  }
}
