...
export class LeaveTypeService {
  ...
  async list(
    ...
    return this.leaveTypeRepository.listWithoutPagination(output, {
      deleted_on: null,
  ...
  async update(
    ...
    if (input.name) {
      ...
      if (leaveType && leaveType.id != id)
        throw new LeaveTypeAlreadyExistException(leaveType[0].id);
    }
  ...
  async delete(current_user: ICurrentUser, id: string): Promise<boolean> {
    const result = await this.leaveTypeRepository.markAsDelete(
