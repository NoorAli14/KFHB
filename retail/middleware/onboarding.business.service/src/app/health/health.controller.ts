...
export class HealthController {
  ...
  healthCheck(): Promise<any> {
    const promises = [
      async (): Promise<any> => this.memory.checkRSS('memory_rss', 3000 * 1024 * 1024),
      async (): Promise<any> => this.memory.checkHeap('memory_heap', 200 * 1024 * 1024),
    ];
    const _promises = this.registry.services.map(service => {
      const host_name = `${service.is_secure ? 'https' : 'http'}://${service.host_name}:${service.port}/api/v1/health`;
