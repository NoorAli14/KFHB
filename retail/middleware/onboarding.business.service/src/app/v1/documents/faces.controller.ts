...
export class FacesController {
  ...
  async uploadSelfie(@CurrentUser() currentUser: User, @Body() input: UploadSelfieDTO): Promise<Session> {
    ...
    if (SELFIE_SUB_TYPES.DEVICE_REGISTRATION === input.sub_type) {
      await this.customerService.updateLastStep(currentUser.id, CUSTOMER_LAST_STEPS.RBX_ONB_STEP_SELFIE_UPLOADED);
    }
    return result;
  }
  ...
  async uploadLiveness(@CurrentUser() currentUser: User, @Body() input: UploadDocumentDTO): Promise<Document> {
    const params: FaceUploadingInput = {
      file: input.file,
    };
    const result = await this.attachmentService.uploadLiveness(params);
    if (result?.status === DOCUMENT_STATUSES.PROCESSED) {
      await this.customerService.updateLastStep(currentUser.id, CUSTOMER_LAST_STEPS.RBX_ONB_STEP_LIVENESS_UPLOADED);
    }
    return result;
  }
}
