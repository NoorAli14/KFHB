import { isArray, isFunction, keys, values, isPlainObject } from 'lodash';

function nestHydrationJS() {
  const NestHydrationJS: {
    typeHandlers: {
      [x: string]: any;
      NUMBER?: (cellValue: any) => number;
      BOOLEAN?: (cellValue: any) => boolean;
    };
    nest: (data: any, structPropToColumnMap: any) => any;
    structPropToColumnMapFromColumnHints: any;
    buildMeta: any;
    registerType: (name: any, handler: any) => void;
  } = {
    typeHandlers: {
     ...
     */
    nest: function(data, structPropToColumnMap) {
      let listOnEmpty, table, meta, struct, i, row, j, _nest, primeIdColumn;
      ...
      if (
        !isArray(structPropToColumnMap) &&
        !isPlainObject(structPropToColumnMap) &&
        structPropToColumnMap !== null &&
        structPropToColumnMap !== true
      ) {
        throw new Error(
          'nest expects param structPropToColumnMap to be an array, plain object, null, or true',
        );
      }
      ...
      if (structPropToColumnMap === null && table.length > 0) {
        ...
        structPropToColumnMap = NestHydrationJS.structPropToColumnMapFromColumnHints(
          keys(table[0]),
        );
      }
      ...
      _nest = function(row, idColumn) {
        ...
        if (value === null) {
          if (
            objMeta.default !== null &&
            typeof objMeta.default !== 'undefined'
          ) {
        ...
        if (typeof objMeta.cache[value] !== 'undefined') {
          ...
          if (
            typeof objMeta.containingIdUsage[value] !== 'undefined' &&
            typeof objMeta.containingIdUsage[value][containingId] !==
              'undefined'
          ) {
     ...
     */
    buildMeta: function(structPropToColumnMap) {
      // internally defines recursive function with extra param. This allows cleaner API
      let meta, _buildMeta, primeIdColumn;

      // recursive internal function
      _buildMeta = function(
        structPropToColumnMap,
        isOneOfMany,
        containingColumn,
        ownProp,
      ) {
        let propList, idProp, idColumn, i, prop, objMeta, subIdColumn;
        ...
        idColumn =
          structPropToColumnMap[idProp].column || structPropToColumnMap[idProp];
        ...
        objMeta = {
          ...
          default:
            typeof structPropToColumnMap[idProp].default === 'undefined'
              ? null
              : structPropToColumnMap[idProp].default,
        };
      ...
      if (isArray(structPropToColumnMap)) {
        ...
        if (typeof primeIdColumn === 'undefined') {
          throw new Error(
            'invalid structPropToColumnMap format - the base object can not be an empty object',
          );
        }
     ...
     * is not specified.
     */
    structPropToColumnMapFromColumnHints: function(columnList, renameMapping) {
      let propertyMapping,
        prop,
        i,
        columnType,
      ...
      for (i = 0; i < columnList.length; i++) {
        ...
        for (j = 1; j < columnType.length; j++) {
          if (columnType[j] === 'ID') {
            ...
            typeof NestHydrationJS.typeHandlers[columnType[j]] !== 'undefined'
          ) {
            type = columnType[j];
          }
        }
        ...
        for (j = 0; j < navList.length; j++) {
          ...
          if (nav === '') {
            ...
            if (typeof pointer[prop][nav] === 'undefined') {
              renamedColumn =
                typeof renameMapping[column] === 'undefined'
                  ? column
              ...
              if (isId) {
                ...
                for (k = 0; k < prevKeyList.length; k++) {
                  if (pointer[prop][prevKeyList[k]].id === true) {
                    return (
                      'invalid - multiple id - ' +
                      pointer[prop][prevKeyList[k]].column +
                      ' and ' +
