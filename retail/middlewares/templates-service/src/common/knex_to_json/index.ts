...
export async function knex_to_json(
  ...
  let structPropToColumnMap = null;

  if (
    knexQuery.client.config
    ...
    if (knexQuery._statements === undefined) {
      return Promise.reject(
        'knex query object not structured as expected for KnexNest: does not have _statements property',
      );
    }
    ...
    let column, alias, prepend, renamed, renamedColumn;

    for (let i = 0; i < knexQuery._statements.length; i++) {
      ...
      if (knexQuery._statements[i].value === undefined) {
        return Promise.reject(
          'knex query object not structured as expected for KnexNest: _statements item with column grouping does not have value property',
        );
      }

      // columns statement, use it
      for (let j = 0; j < knexQuery._statements[i].value.length; j++) {
        renamedColumn = null;
        ...
        if (column.substr(-1) === '"') {
          ...
          alias = column.slice(
            column.lastIndexOf('"', column.length - 2) + 1,
            -1,
          );

          if (alias.length > MAX_POSTGRES_COLUMN_NAME_LENGTH) {
            // shorten the alias to allowed size
            prepend = 'col_' + uniqueId++ + '_';
            renamed =
              prepend +
              alias.substr(
                -1 * MAX_POSTGRES_COLUMN_NAME_LENGTH + prepend.length,
              );
            ...
            renamedColumn =
              column.substr(0, column.indexOf('"')) + '"' + renamed + '"';
          }
          ...
          if (alias.length > MAX_POSTGRES_COLUMN_NAME_LENGTH) {
            ...
            // replace the original alias with the shortened one
            renamedColumn =
              column.substr(0, column.lastIndexOf(' ') + 1) + renamed;
          }
          aliasList.push(alias);
    ...
    if (!(structPropToColumnMap instanceof Array) && listOnEmpty) {
      return Promise.reject(
        'listOnEmpty param conflicts with query which specifies an object or null result',
      );
    }
  }
...
export const MAX_POSTGRES_COLUMN_NAME_LENGTH = 63;
